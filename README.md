...
Web editor for trimming robot end-effector subtrees, adding fixed/TCP links, and exporting URDF.

一个轻量级 Web URDF 末端执行器编辑器，用于裁剪机器人末端子树、添加固定连杆/TCP，并导出修改后的 URDF。

This workspace is based on [fan-ziqi/robot_viewer](https://github.com/fan-ziqi/robot_viewer) and keeps the upstream Apache-2.0 license and attribution. The current product direction is a browser-based URDF end-effector editor: stable URDF + meshes import first, then link/joint inspection, descendant subtree preview, TCP insertion, and URDF export.

The upstream README is preserved below for attribution and feature context.
