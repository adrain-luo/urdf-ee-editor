...
export class CodeEditorManager {
    ...
    getCurrentContent() {
        ...
        return this.editorState.currentContent || '';
    }

    getCurrentFileName(fallback = 'edited_robot.urdf') {
        return this.editorState.currentFile?.name || this.editorState.defaultFileName || fallback;
    }

    async replaceContentAndReload(newContent) {
        if (!this.codeEditorInstance || !newContent) return;

        const currentFile = this.editorState.currentFile;
        const fileName = currentFile?.name || this.editorState.defaultFileName || 'edited_robot.urdf';
        const fileType = currentFile?.type || this.detectFileTypeFromName(fileName);
        const blob = new Blob([newContent], { type: 'text/plain' });
        ...
        if (this.fileMap) {
            ...
            for (const [key, value] of this.fileMap.entries()) {
                if (value === currentFile || key === fileName || key.endsWith('/' + fileName)) {
                    this.fileMap.set(key, newFile);
                    replaced = true;
                }
            }
        ...
        this.codeEditorInstance.setValue(newContent);
        this.editorState.currentFile = newFile;
    ...
    downloadCurrentContent(fileName = null) {
        const name = fileName || this.getCurrentFileName();
        this.downloadFile(this.getCurrentContent(), name);
    }

    downloadContent(content, fileName = null) {
        const name = fileName || this.getCurrentFileName();
