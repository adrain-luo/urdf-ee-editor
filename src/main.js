...
import { EndEffectorEditorController } from './controllers/EndEffectorEditorController.js';
...
class App {
    constructor() {
        ...
        this.endEffectorEditorController = null;
    ...
    async init() {
        try {
            ...
            // Initialize end-effector editor controller
            this.endEffectorEditorController = new EndEffectorEditorController({
                sceneManager: this.sceneManager,
                modelGraphView: this.modelGraphView,
                codeEditorManager: this.codeEditorManager
            });

            if (this.modelGraphView) {
                this.modelGraphView.setLinkSelectionCallback((linkName, options) => {
                    this.endEffectorEditorController?.selectLink(linkName, options);
                });
            }
    ...
    async handleModelLoaded(model, file, isMesh = false, snapshot = null) {
        ...
        this.currentModel = model;
        if (this.endEffectorEditorController) {
            this.endEffectorEditorController.setModel(model);
        }
        ...
        canvas.addEventListener('mouseup', (event) => {
            ...
            if (distance < 5 && duration < 300) {
                ...
                if (modelIntersects.length === 0) {
                    ...
                    this.endEffectorEditorController?.clearSelection();
                    ...
                    const linkName = this.endEffectorEditorController?.findLinkNameFromObject(modelIntersects[0].object);
                    if (linkName) {
                        if (this.measurementController) {
                            this.measurementController.clearMeasurement();
                        }
                        this.endEffectorEditorController.selectLink(linkName, { source: 'viewport' });
                    }
                }
            }
