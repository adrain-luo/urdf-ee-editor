...
export class ModelGraphView {
    ...
    drawModelGraph(model) {
        ...
        node.on('click', (event, d) => {
            ...
            if (event.ctrlKey || event.metaKey) {
                ...
                if (this.onLinkSelected && d.data.name) {
                    this.onLinkSelected(d.data.name, { source: 'graph' });
                } else {
    ...
    setLinkSelectionCallback(callback) {
        this.onLinkSelected = callback;
    }

    selectLinkByName(linkName) {
        if (!this.currentSvg || !linkName) {
            return false;
        }
