...
export class SceneManager {
    ...
    addModel(model) {
        ...
        if (isSingleMesh) {
            ...
            // Complete robot model (URDF/MJCF/USD): keep a world object for optional up-axis changes.
            // The default +Z mode is identity so URDF / ROS coordinates stay native.
            if (!this.world) {
    ...
    updateEnvironment(fitCamera = false) {
        ...
        const minZ = bboxGlobal.min.z;  // URDF / ROS Z-up scene: Z is vertical

        // Update ground position to model lowest point (robot touches ground)
        let groundChanged = false;
        if (this.groundPlane) {
            const newGroundZ = minZ;  // Move ground to robot lowest point
            const oldGroundZ = this.groundPlane.position.z;
            this.groundPlane.position.z = newGroundZ;

            // Also update grid position, keep aligned with ground
            if (this.referenceGrid) {
                this.referenceGrid.position.z = newGroundZ;
                this.referenceGrid.updateMatrixWorld(true);
            }

            // Detailed debug info
            // If ground position changed, mark for measurement update
            if (Math.abs(oldGroundZ - newGroundZ) > 1e-6) {
                groundChanged = true;
            }
        }
    ...
    fitCameraToModel(bbox, center, size) {
        ...
        // ROS-friendly oblique view:
        // - From front-right (X positive, Y negative)
        // - Slightly looking down (Z positive)
    ...
    setUp(up) {
        ...
        if (axis === 'X') {
            this.world.rotation.set(0, sign === '+' ? -HALFPI : HALFPI, 0);
        } else if (axis === 'Z') {
